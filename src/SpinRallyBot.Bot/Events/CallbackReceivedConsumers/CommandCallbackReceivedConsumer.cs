...
public class CommandCallbackReceivedConsumer : IMediatorConsumer<CallbackReceived> {
    private readonly ILogger<CommandCallbackReceivedConsumer> _logger;
    private readonly IScopedMediator _mediator;

    public CommandCallbackReceivedConsumer(IScopedMediator mediator,
        ILogger<CommandCallbackReceivedConsumer> logger) {
        _mediator = mediator;
        _logger = logger;
    }
    ...
    public async Task Consume(ConsumeContext<CallbackReceived> context) {
        ...
        await _mediator.Publish(new CommandReceived(command, args, chatId, chatType, messageId, userId), context.CancellationToken);
    }
}